...
@Component({
  selector: "app-confirm-other-document",
  templateUrl: "./confirm-other-document.component.html",
  styleUrls: ["./confirm-other-document.component.scss"],
})
export class ConfirmOtherDocumentComponent implements OnInit {
  constructor(
  ...
  selectOtherDocumentArray: OtherFileModel[] = [];
  ...
  ngOnInit() {
    this.otherDocumentSearchDto.state = "SENT";
  ...
  confirmDoc() {
    swal
      .fire({
        title: "آیا تایید سایر اسناد را تایید می کنید؟",
      ...
      .then((result) => {
        if (result.value) {
          let request = {
            ids: this.selectOtherDocumentArray.map((item) => item.id),
            operation: "confirm",
          };
          this.otehrDocumentService.confirmOtherDocument(request).then(() => {
            this.loadOtherDocument({ first: this.currentPage * 10, rows: 10 });
            this.toastr.success("تایید سایر اسناد با موفقیت انجام شد");
            this.selectOtherDocumentArray = [];
          });
        }
      })
      .catch(() =>
